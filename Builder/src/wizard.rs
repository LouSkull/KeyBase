...
impl WizardState {
    ...
    pub fn effective_fields(&self) -> Vec<FieldId> {
        ...
        // Update: mode + path + optional security operations
...
pub fn auto_find_installs() -> Vec<String> {
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            ...
            candidates.push(home.join("apps").join("keybase").to_string_lossy().into_owned());
            candidates.push(home.join(".local").join("share").join("keybase").to_string_lossy().into_owned());
        }
