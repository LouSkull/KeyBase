...
impl InstallMode {
    ...
    pub fn all() -> &'static [InstallMode] {
        &[InstallMode::Fresh, InstallMode::Update, InstallMode::Uninstall]
    }
}
...
impl WizardState {
    ...
    pub fn to_config(&self) -> BuildConfig {
        let (jwt_secret, admin_password, api_master_key) =
            if matches!(self.install_mode, InstallMode::Update | InstallMode::Uninstall) {
    ...
    pub fn effective_fields(&self) -> Vec<FieldId> {
        // Update / Uninstall: only mode selector + path
        if matches!(self.install_mode, InstallMode::Update | InstallMode::Uninstall) {
...
pub fn auto_find_installs() -> Vec<String> {
    ...
    #[cfg(windows)]
    {
        for drive in &["C", "D", "E", "F", "G"] {
            candidates.push(format!("{}:\\KeyBase", drive));
        }
        if let Ok(user) = std::env::var("USERPROFILE") {
            candidates.push(format!("{}\\KeyBase", user));
        }
    ...
    #[cfg(not(windows))]
    {
        if let Some(home) = dirs::home_dir() {
            candidates.push(home.join("keybase").to_string_lossy().into_owned());
            candidates.push(home.join("KeyBase").to_string_lossy().into_owned());
        }
        candidates.push("/opt/keybase".to_string());
        candidates.push("/opt/KeyBase".to_string());
        candidates.push("/srv/keybase".to_string());
        candidates.push("/var/lib/keybase".to_string());
    }
