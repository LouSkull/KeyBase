...
impl TextInput {
    pub fn new(s: &str) -> Self {
        Self { buf: s.to_string(), cursor: s.len() }
    }
    ...
    pub fn move_right(&mut self) {
        ...
        if self.cursor < l { self.cursor += 1; }
    }
    pub fn home(&mut self) { self.cursor = 0; }
    pub fn end(&mut self) { self.cursor = self.buf.chars().count(); }
...
impl WizardState {
    ...
    pub fn to_config(&self) -> BuildConfig {
        BuildConfig {
            ...
            jwt_secret: self.f_jwt_secret.buf.clone(),
            admin_password: self.f_admin_password.buf.clone(),
            api_master_key: self.f_api_master_key.buf.clone(),
            debug: self.debug,
