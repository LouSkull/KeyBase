...
use egui::{
    Align, Color32, FontId, Layout, RichText, Rounding, ScrollArea, Stroke, Vec2,
};
...
use crate::{
    ...
    selftest::TestResult,
    wizard::{DbBackend, InstallMode, LogLevel, TargetOs},
};
...
impl eframe::App for GuiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::bottom("bottombar")
            .exact_height(54.0)
            .frame(egui::Frame::none().fill(SIDEBAR).inner_margin(egui::Margin::symmetric(16.0, 8.0)))
            .show(ctx, |ui| {
...
fn render_sidebar(ui: &mut egui::Ui, app: &App) {
    ...
    for (idx, name) in steps {
        ...
        let icon = if is_done { "✓" } else if is_active { "▶" } else { "○" };
        ...
        let bg = if is_active { Color32::from_rgb(38, 44, 56) } else { Color32::TRANSPARENT };
    ...
    let phase_label = match app.phase {
        ...
        _            => RichText::new(format!("  Step {}/6", phase_idx + 1)).color(DIM).size(12.0),
    };
        ...
        match app.phase {
            ...
            Phase::Wizard => {
                ...
                let (ready, warn_msg) = if is_update {
                    // Update mode: secrets are preserved; only check enabled ops
                    ...
                    let ok = pwd_ok && jwt_ok && api_ok && name_ok;
                    (ok, "⚠ Fill enabled Security Operations")
                } else {
                    let ok = ws.f_jwt_secret.buf.len() >= 32
                        && ws.f_admin_password.buf.len() >= 8
                        && ws.f_api_master_key.buf.len() >= 16;
                    (ok, "⚠ Fill Security & API Keys")
                };
...
fn render_syscheck(ui: &mut egui::Ui, app: &App) {
    ...
    egui::Grid::new("syschecks")
        ...
        .show(ui, |ui| {
            for check in &info.checks() {
                ...
                ui.label(RichText::new(&check.value).color(match check.status {
    ...
    ui.add_space(16.0);
    if info.required_ok() {
        ui.label(RichText::new("✓ All checks passed. Ready to proceed.").color(OK).size(14.0));
    } else {
        ui.label(RichText::new("✗ Required dependencies are missing.").color(ERR).size(14.0));
        ui.add_space(6.0);
        ui.label(RichText::new("Install Python 3.10+ from https://python.org/downloads").color(DIM));
    }
}
...
fn render_wizard(ui: &mut egui::Ui, app: &mut App) {
    ...
    ScrollArea::vertical().show(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ui.set_height(28.0);
            ui.label(RichText::new("Output directory").color(DIM).size(13.0));
            ui.add_space(8.0);
            let resp = ui.add(
        ...
        // Install mode — smart: rename/disable based on detected install
        {
            // Lazily detect on first render if not yet checked
            ...
            field_row(ui, "Install mode", |ui| {
                ...
                ui.add_enabled(update_enabled, egui::SelectableLabel::new(
                    ws.install_mode == InstallMode::Update,
                    "Update",
                )).clicked().then(|| {
                    if update_enabled { ws.install_mode = InstallMode::Update; }
                });
                if !update_enabled {
        ...
        if is_update {
            section_heading(ui, "Security & API Keys");
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.add_space(6.0);
                        ui.vertical(|ui| {
                            ui.label(RichText::new("Secrets preserved from existing install").color(OK).size(13.0).strong());
                            ui.label(RichText::new(
                                "JWT Secret, Admin Password and API Master Key are NOT changed during update.\n\
                                 Use Security Operations below to reset specific credentials."
                            ).color(DIM).size(11.0));
                        });
                    });
                });
        } else {
            section_heading(ui, "Security & API Keys  ⚠ required");
            ui.label(RichText::new(
                "  Written to .env.example and config.yml. Keep them safe."
            ).color(DIM).size(11.0));
            ui.add_space(4.0);

            secret_row(ui, "JWT Secret",
                "min 32 chars — signs all auth tokens", 32,
                &mut ws.f_jwt_secret.buf, &mut ws.show_jwt,
            );
            secret_row(ui, "Admin Password",
                "min 8 chars — initial admin account", 8,
                &mut ws.f_admin_password.buf, &mut ws.show_admin_pass,
            );
            secret_row(ui, "API Master Key",
                "min 16 chars — protects management endpoints", 16,
                &mut ws.f_api_master_key.buf, &mut ws.show_api_key,
            );
        }
        ...
        if is_update {
            ...
            ui.label(RichText::new(
                "  Reset or rotate credentials without reinstalling the server."
            ).color(DIM).size(11.0));
            ui.add_space(6.0);
...
fn secret_row(
    ...
    ui.horizontal(|ui| {
        ...
        ui.add_space(4.0);
        if ok {
            ui.label(RichText::new(format!("✓ {} chars", value.len())).color(OK).size(11.0));
        } else if value.is_empty() {
            ui.label(RichText::new("required").color(ERR).size(11.0));
        } else {
            ui.label(RichText::new(format!("{}/{} chars", value.len(), min_len)).color(WARN).size(11.0));
        }
    });
        ...
        ui.horizontal(|ui| {
            ...
            if !value.is_empty() {
                if value.len() >= min_len {
                    ...
                    ui.label(RichText::new(format!("{}/{}", value.len(), min_len)).color(WARN).size(11.0));
                }
            }
        });
        ui.add_space(2.0);
...
fn field_row(ui: &mut egui::Ui, label: &str, content: impl FnOnce(&mut egui::Ui)) {
    ui.horizontal(|ui| {
        ...
        ui.add(egui::Label::new(
            RichText::new(label).color(DIM).size(13.0),
        ).wrap());
        ui.add_space(8.0);
    ...
    ScrollArea::vertical().show(ui, |ui| {
        ...
        egui::Grid::new("confirm_grid")
            ...
            .show(ui, |ui| {
                ...
                row(ui, "Allow remote admin",  if cfg.allow_remote_admin {"Yes"} else {"No"});
                row(ui, "Trust proxy",         if cfg.trust_proxy {"Yes"} else {"No"});
                row(ui, "Cloudflare mode",     if cfg.cloudflare {"Yes"} else {"No"});
                if !cfg.domain.is_empty() {
                    row(ui, "Domain",          &cfg.domain);
                }
                row(ui, "API rate limit",      &format!("{}/min", cfg.rate_limit));
                row(ui, "Provisioning API",    if cfg.prov_enabled {"Enabled"} else {"Disabled"});
                row(ui, "Target OS",           cfg.target_os.label());
                row(ui, "Backup interval",     &format!("{}m", cfg.backup_interval_u32()));
                row(ui, "Backups to keep",     &cfg.backup_keep_u32().to_string());
                row(ui, "Session hours",       &cfg.session_hours_u32().to_string());
                row(ui, "Docker files",        if cfg.gen_docker {"Yes"} else {"No"});
                row(ui, "NGINX config",        if cfg.gen_nginx  {"Yes"} else {"No"});
                row(ui, "systemd service",     if cfg.gen_systemd{"Yes"} else {"No"});
        ...
        ui.separator();
        ui.add_space(8.0);
        ui.label(RichText::new(format!("Admin panel: http://{}:{}/admin", cfg.host, port)).color(ACCENT));
        ui.add_space(4.0);
        ui.label(RichText::new("Click \"Install Now\" to begin installation.").color(DIM).size(13.0));
    });
}
...
fn render_installing(ui: &mut egui::Ui, install: &InstallState) {
    ...
    ui.label(RichText::new("Installing…").color(ACCENT).size(15.0).strong());
    ui.add_space(12.0);
    ...
    for (step, label) in steps {
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(RichText::new(*label).color(if active { Color32::WHITE } else { DIM }).size(13.0));
            ...
            if active && *step == InstallStep::Downloading && install.total > 0 {
                ...
                ui.label(RichText::new(format!("  {:.1} / {:.1} MB", done_mb, total_mb)).color(DIM).size(12.0));
            }
        });
    }
    ...
    ScrollArea::vertical()
        ...
        .show(ui, |ui| {
            ...
            for line in &install.log {
                ui.add(egui::Label::new(
                    RichText::new(line).color(DIM).font(FontId::monospace(11.0))
                ).wrap());
            }
        });
}
...
fn render_selftest(ui: &mut egui::Ui, app: &App) {
    ui.label(RichText::new("Running Self-Test…").color(ACCENT).size(15.0).strong());
    ui.add_space(16.0);
...
fn render_done(ui: &mut egui::Ui, app: &App) {
    ...
    ui.horizontal_top(|ui| {
        ...
        ui.vertical(|ui| {
            ...
            ui.label(RichText::new("Installation Complete").color(OK).size(18.0).strong());
            ui.add_space(12.0);
            ...
            if let Some(result) = &app.test_result {
                ui.add_space(4.0);
                match result {
                    TestResult::Pass    => { success_row(ui, "Self-test PASSED"); }
                    TestResult::Fail(e) => { ui.label(RichText::new(format!("Self-test FAILED: {e}")).color(ERR)); }
                    TestResult::Skipped => { ui.label(RichText::new("Self-test skipped").color(DIM)); }
                }
            }
            ...
            kv_row(ui, "Admin panel", &format!("http://{}:{}/admin", host, port));
            ...
            if app.config.target_os.gen_bat() {
                ui.label(RichText::new("  Windows:  run.bat").color(Color32::WHITE).font(FontId::monospace(12.0)));
            }
            if app.config.target_os.gen_sh() {
                ui.label(RichText::new("  Linux:    ./run.sh").color(Color32::WHITE).font(FontId::monospace(12.0)));
            }

            ui.add_space(12.0);
            ui.label(RichText::new("On first run, create your admin account.").color(DIM).size(12.0));
        });
        ...
        ui.vertical(|ui| {
            ui.label(RichText::new("Generated files:").color(ACCENT).size(13.0).strong());
            ui.add_space(8.0);

            let base = ["config.yml", ".env.example", "README.md", "install.log"];
            for f in &base {
                file_row(ui, f);
            }
...
fn kv_row(ui: &mut egui::Ui, key: &str, value: &str) {
    ui.horizontal(|ui| {
        ui.label(RichText::new(format!("  {:<14}", key)).color(DIM).size(13.0));
        ui.label(RichText::new(value).color(Color32::WHITE).strong().size(13.0));
    });
}
...
fn render_error(ui: &mut egui::Ui, app: &App) {
    ui.label(RichText::new("Installation Failed").color(ERR).size(18.0).strong());
    ui.add_space(16.0);
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ui.label(RichText::new(&app.error_msg).color(Color32::from_rgb(255, 160, 160)).font(FontId::monospace(12.0)));
        });

    ui.add_space(16.0);
    ui.label(RichText::new("Check your settings and try again.").color(DIM).size(13.0));
}
