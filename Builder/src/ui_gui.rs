...
use crate::{
    ...
    selftest::TestResult,
    wizard::{auto_find_installs, DbBackend, InstallMode, LogLevel},
};
...
impl eframe::App for GuiApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        // Custom frameless title bar — drag + window controls on the right
        egui::TopBottomPanel::top("topbar")
            ...
            .show(ctx, |ui| {
                ...
                // Close button — 22 px from right edge
                let close_c = egui::pos2(rect.right() - 22.0, rect.center().y);
                ...
                let close_clr = if close.hovered() {
                    Color32::from_rgb(230, 75, 65)
                } else {
                    Color32::from_rgb(255, 95, 87)
                };
                ui.painter().circle_filled(close_c, 7.0, close_clr);
                if close.clicked() {
                    if self.app.is_busy() {
                ...
                // Minimize button — 50 px from right edge
                let min_c = egui::pos2(rect.right() - 50.0, rect.center().y);
                let min_r = egui::Rect::from_center_size(min_c, Vec2::splat(18.0));
                let min_resp = ui.interact(min_r, ui.id().with("min_btn"), egui::Sense::click());

                let min_clr = if min_resp.hovered() {
                    Color32::from_rgb(200, 155, 20)
                } else {
                    Color32::from_rgb(255, 189, 46)
                };
                ui.painter().circle_filled(min_c, 7.0, min_clr);
                if min_resp.clicked() {
...
fn paint_icon(ui: &mut egui::Ui, icon: UiIcon, color: Color32) {
    ...
    match icon {
        UiIcon::Done => {
            painter.line_segment(
                [
                    ...
                    egui::pos2(rect.left() + 5.4, rect.bottom() - 3.0),
                ],
                stroke,
            );
            painter.line_segment(
                [
                    egui::pos2(rect.left() + 5.4, rect.bottom() - 3.0),
                    egui::pos2(rect.right() - 2.0, rect.top() + 3.0),
                ],
                stroke,
            );
        }
        UiIcon::Active => {
            painter.add(egui::Shape::convex_polygon(
                vec![
                    ...
                    egui::pos2(rect.right() - 2.5, center.y),
        ...
        UiIcon::Warn => {
            painter.add(egui::Shape::closed_line(
                vec![
                    egui::pos2(center.x, rect.top() + 1.5),
            ...
            painter.circle_filled(egui::pos2(center.x, rect.bottom() - 3.0), 1.1, color);
        }
        UiIcon::Error => {
            painter.line_segment(
                [
...
fn render_footer(ui: &mut egui::Ui, app: &mut App) {
    ui.with_layout(Layout::right_to_left(Align::Center), |ui| match app.phase {
        Phase::Welcome => {
        ...
        Phase::Wizard => {
            ...
            let (ready, warn_msg) = if is_update {
                let pwd_ok = !ws.sec_reset_pwd || ws.f_sec_pwd.buf.len() >= 8;
                let jwt_ok = !ws.sec_reset_jwt || ws.f_sec_jwt.buf.len() >= 32;
                let api_ok = !ws.sec_reset_api || ws.f_sec_api.buf.len() >= 16;
                let name_ok = !ws.sec_change_username || ws.f_sec_username.buf.len() >= 3;
                let prov_ok = !ws.prov_enabled || !ws.f_prov_token.buf.trim().is_empty();
                let ok = pwd_ok && jwt_ok && api_ok && name_ok && prov_ok;
                let warn = if !prov_ok {
                    "WARNING: Provision token cannot be empty while provisioning is enabled"
                } else {
                    "WARNING: Fill enabled Security Operations"
                };
                (ok, warn)
        ...
        Phase::Confirm => {
            match app.config.install_mode {
                ...
                InstallMode::Update => {
                    if primary_btn_colored(ui, "Update Now", ACCENT) {
                        app.start_install();
                    }
                }
                InstallMode::Fresh => {
                    if primary_btn_colored(ui, "Install Now", OK) {
                        app.start_install();
                    }
                }
            }
        ...
        Phase::Installing | Phase::SelfTest => {
            if app.cancel_requested {
                ui.label(RichText::new("Cancelling...").color(WARN).italics());
            } else if secondary_btn(ui, "Cancel") {
                app.request_cancel();
            }
        }
        Phase::Done => {
            if app.config.output_path().exists() && primary_btn(ui, "Open Folder") {
...
fn render_body(ui: &mut egui::Ui, app: &mut App) {
    match app.phase {
        ...
        Phase::Installing => render_installing(ui, app),
...
fn render_syscheck(ui: &mut egui::Ui, app: &App) {
        ...
        .show(ui, |ui| {
            for check in &info.checks() {
                let (icon, color) = match check.status {
                    CheckStatus::Ok => (UiIcon::Done, OK),
                    CheckStatus::Warn => (UiIcon::Warn, WARN),
                    CheckStatus::Fail => (UiIcon::Error, ERR),
                };
                paint_icon(ui, icon, color);
    ...
    if info.required_ok() {
        ...
        ui.label(
            RichText::new("Required checks are failing.")
                .color(ERR)
                .size(14.0),
        );
        if !info.elevated {
            ui.add_space(6.0);
            ui.label(RichText::new(&info.elevation_hint).color(WARN));
        }
...
fn render_wizard(ui: &mut egui::Ui, app: &mut App) {
    ...
    ScrollArea::vertical().show(ui, |ui| {
            ...
            field_row(ui, "Mode", |ui| {
                ...
                if ui.add_enabled(update_enabled, egui::SelectableLabel::new(
                    ...
                    if let Some(p) = found.into_iter().next() {
                        ...
                        ws.output_autofill_gen = ws.output_autofill_gen.wrapping_add(1);
                    }
                }
                ui.add_space(6.0);
                if ui.add_enabled(update_enabled, egui::SelectableLabel::new(
        ...
        ui.horizontal(|ui| {
            ...
            ui.add_space(8.0);
            // Auto-find rendered first so its buf update is visible to TextEdit below in same frame
            if ui.button("Auto-find").clicked() {
                ...
                if let Some(p) = found.first() {
                    ...
                    // Fresh mode: suggest platform default if nothing found
                    ws.f_output.buf = if cfg!(windows) {
                        "C:\\KeyBase".to_string()
                    } else {
                        "/opt/keybase".to_string()
                    };
                    ws.dir_has_install = Some(false);
                }
                ws.output_autofill_gen = ws.output_autofill_gen.wrapping_add(1);
            }
            ui.add_space(4.0);
            let resp = ui.add(
                egui::TextEdit::singleline(&mut ws.f_output.buf)
                    .id(egui::Id::new("output_path_edit").with(ws.output_autofill_gen))
        ...
        if is_update {
            ...
            sec_op_row(ui,
                ...
                &mut ws.f_sec_pwd.buf,
                &mut ws.sec_show_new_pwd,
            );
            sec_op_row(ui,
                "Rotate JWT Secret",
                "WARNING: Invalidates ALL active sessions - every user will be logged out!",
                true,
                32,
                &mut ws.sec_reset_jwt,
                &mut ws.f_sec_jwt.buf,
                &mut ws.sec_show_new_jwt,
            );
            sec_op_row(ui,
                "Rotate API Master Key",
                "WARNING: Breaks all integrations using the current key until they update!",
                true,
                16,
                &mut ws.sec_reset_api,
                &mut ws.f_sec_api.buf,
            ...
            if ws.sec_change_username {
                ui.horizontal(|ui| {
                    ...
                    ui.add_space(8.0);
                    ui.add(egui::TextEdit::singleline(&mut ws.f_sec_username.buf)
                        .desired_width(200.0)
                        .hint_text("e.g. admin"));
                    if ws.f_sec_username.buf.len() < 3 && !ws.f_sec_username.buf.is_empty() {
            ...
            // ── Update mode: generated files ──────────────────────────────────
            section_heading(ui, "Generated Files");
            ui.label(RichText::new(
                "  Optionally regenerate extra files during this update."
            ).color(DIM).size(11.0));
            ui.add_space(4.0);
            ...
            toggle_row(ui, "Run self-test after update", &mut ws.selftest);
        }
        ...
        if is_uninstall {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ui.set_width(ui.available_width());
                    ui.label(RichText::new("WARNING: Permanently removes the installation directory and all its contents.").color(ERR).size(13.0).strong());
                    ui.add_space(4.0);
...
fn secret_row(
    ...
    ui.horizontal(|ui| {
        ...
        if ok {
            ui.label(
                RichText::new(format!("OK {} chars", value.len()))
                    .color(OK)
                    .size(11.0),
            );
        ...
        ui.horizontal(|ui| {
            ...
            if !value.is_empty() {
                if value.len() >= min_len {
                    ui.label(RichText::new("OK").color(OK).size(12.0));
                } else {
                    ui.label(
...
fn render_confirm_uninstall(ui: &mut egui::Ui, app: &App) {
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ui.set_width(ui.available_width());
            ui.label(
                RichText::new("The following directory will be PERMANENTLY DELETED:")
                    .color(ERR)
                    .size(14.0)
                    .strong(),
            );
            ui.add_space(8.0);
            ui.label(
                RichText::new(&app.config.output_dir)
                    .color(Color32::WHITE)
                    .size(14.0)
                    .strong()
            ...
            ui.label(
                RichText::new(
                    "This includes the database, all keys, config, backups, and server files.",
                )
                .color(WARN)
                .size(12.0),
            );
    ...
    ui.add_space(12.0);
    ui.label(
        RichText::new("Click \"Uninstall\" to proceed, or <- Back to cancel.")
            .color(DIM)
            .size(13.0),
    );
}

fn render_confirm_update(ui: &mut egui::Ui, app: &App) {
    section_heading(ui, "Confirm Update");
    ScrollArea::vertical().show(ui, |ui| {
        let cfg = &app.config;

        egui::Grid::new("confirm_update_grid")
            .num_columns(2)
            .spacing([16.0, 6.0])
            .show(ui, |ui| {
                ...
                row(ui, "Directory", &cfg.output_dir);
                row(ui, "Mode", cfg.install_mode.label());
            });

        // What changes
        ui.add_space(10.0);
        egui::Frame::none()
            .fill(Color32::from_rgb(22, 35, 25))
            .inner_margin(egui::Margin::same(10.0))
            .rounding(Rounding::same(4.0))
            .show(ui, |ui| {
                ui.set_width(ui.available_width());
                ui.label(
                    RichText::new("Will be replaced:")
                        .color(WARN)
                        .size(12.0)
                        .strong(),
                );
                ui.label(
                    RichText::new("  - Server code files and Python packages")
                        .color(DIM)
                        .size(12.0),
                );
                ui.add_space(4.0);
                ui.label(
                    RichText::new("Will be preserved:")
                        .color(OK)
                        .size(12.0)
                        .strong(),
                );
                ui.label(
                    RichText::new("  - config.yml, .env, data/, backups/")
                        .color(OK)
                        .size(12.0),
                );
            });
        ...
        if any_sec {
            ui.add_space(10.0);
            ui.label(
                RichText::new("Security operations:")
                    .color(WARN)
                    .size(13.0)
                    .strong(),
            );
            ...
            if cfg.sec_change_username {
                ui.label(
                    RichText::new(format!("  Username -> {}", cfg.sec_new_username))
                        .color(DIM)
                        .size(12.0),
                );
            }
            if cfg.sec_reset_pwd {
                ui.label(
                    RichText::new("  Admin password will be reset")
                        .color(DIM)
                        .size(12.0),
                );
            }
            if cfg.sec_reset_jwt {
                ui.label(
                    RichText::new("  Session secret rotated - all users will be logged out")
                        .color(WARN)
                        .size(12.0),
                );
            }
            if cfg.sec_reset_api {
                ui.label(
                    RichText::new("  API master key rotated - update all integrations")
                        .color(WARN)
                        .size(12.0),
                );
            }
        ...
        if any_gen {
            ...
            if cfg.gen_docker {
                ui.label(
                    RichText::new("  - Docker files (Dockerfile, compose)")
                        .color(DIM)
                        .size(12.0),
                );
            }
            ...
            if cfg.gen_systemd {
                ui.label(
                    RichText::new("  - systemd service unit")
                        .color(DIM)
                        .size(12.0),
                );
            }
        ...
        ui.add_space(8.0);
        ui.label(
            RichText::new("Click \"Update Now\" to proceed.")
                .color(DIM)
                .size(13.0),
        );
    });
...
fn render_confirm(ui: &mut egui::Ui, app: &App) {
    match app.config.install_mode {
        InstallMode::Uninstall => {
            render_confirm_uninstall(ui, app);
            return;
        }
        InstallMode::Update => {
            render_confirm_update(ui, app);
            return;
        }
        InstallMode::Fresh => {}
    }
...
fn render_installing(ui: &mut egui::Ui, app: &App) {
    let install = &app.install;
    let ratio = install.overall_progress() as f32;
    let pct = (ratio * 100.0) as u32;

    ui.label(
        RichText::new(if app.config.install_mode == InstallMode::Uninstall {
            "Uninstalling..."
        } else {
            "Installing..."
        })
    ...
    if app.cancel_requested {
        ui.add_space(6.0);
        ui.label(
            RichText::new("Cancellation requested. Waiting for the current step to stop safely.")
                .color(WARN)
                .size(12.0),
        );
    }
...
fn render_selftest(ui: &mut egui::Ui, app: &App) {
    ...
    if app.cancel_requested {
        ui.add_space(6.0);
        ui.label(
            RichText::new("Cancellation requested. Stopping the temporary server...")
                .color(WARN)
                .size(12.0),
        );
    }
...
fn render_done(ui: &mut egui::Ui, app: &App) {
    if app.install_cancelled {
        ...
        ui.add_space(20.0);
        ui.vertical_centered(|ui| {
            ui.label(RichText::new(action).color(WARN).size(20.0).strong());
            ui.add_space(16.0);
            ui.label(
                RichText::new(format!("Path: {}", app.config.output_dir))
                    .color(Color32::WHITE)
                    .size(14.0),
            );
            ui.add_space(8.0);
            ui.label(RichText::new(note).color(DIM).size(13.0));
            if app.build_elapsed > 0.0 {
                ui.add_space(8.0);
                ui.label(
                    RichText::new(format!("Elapsed: {:.1}s", app.build_elapsed))
                        .color(DIM)
                        .size(12.0),
                );
            }
    ...
    if app.config.install_mode == InstallMode::Uninstall {
        ...
        ui.vertical_centered(|ui| {
            ui.label(
                RichText::new("Uninstallation Complete")
                    .color(OK)
                    .size(20.0)
                    .strong(),
            );
            ui.add_space(16.0);
            ui.label(
                RichText::new(format!("Removed: {}", app.config.output_dir))
                    .color(OK)
                    .size(14.0),
            );
            ui.add_space(8.0);
            ui.label(
                RichText::new("All server files, data, and config have been deleted.")
                    .color(DIM)
                    .size(13.0),
            );
        });
            ...
            if let Some(result) = &app.test_result {
                ...
                match result {
                    ...
                    TestResult::Cancelled => {
                        ui.label(RichText::new("Self-test CANCELLED").color(WARN));
                    }
                    TestResult::Fail(e) => {
...
pub fn run() -> anyhow::Result<()> {
    if let Some(msg) = crate::platform::launch_elevation_error() {
        let _ = rfd::MessageDialog::new()
            .set_title("Administrator Rights Required")
            .set_description(&msg)
