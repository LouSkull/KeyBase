...
use crate::{
    ...
    wizard::{
        ...
        InstallMode, LogLevel, TargetOs, WizardState,
    },
};
...
impl App {
    ...
    fn selector_prev(&mut self, fid: FieldId) {
        match fid {
            FieldId::InstallMode => {
                ...
                let cur = modes.iter().position(|&m| m == self.wizard.install_mode).unwrap_or(0);
    ...
    fn auto_populate_path_for_mode(&mut self) {
        if matches!(self.wizard.install_mode, InstallMode::Update | InstallMode::Uninstall) {
    ...
    pub fn start_install(&mut self) {
        ...
        if matches!(cfg.install_mode, InstallMode::Uninstall) {
            cfg.selftest = false;
            let (jh, rx) = install::spawn_uninstall(cfg);
