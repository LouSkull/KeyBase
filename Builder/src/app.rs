...
impl App {
    ...
    fn toggle_or_advance(&mut self, fid: FieldId, _n: usize) {
        match fid {
            ...
            FieldId::Theme => self.wizard.dark_theme = !self.wizard.dark_theme,
            _ => {}
        }
    ...
    fn go_confirm(&mut self) {
        ...
        if matches!(cfg.install_mode, InstallMode::Fresh) && !cfg.secrets_valid() {
            self.error_msg = "Fresh install requires JWT Secret (32+ chars), Admin Password (8+ chars), and API Master Key (16+ chars). Fill them manually or press F6 on each secret field.".into();
        ...
        if matches!(cfg.install_mode, InstallMode::Update)
            ...
            && !cfg.secrets_valid()
        {
            self.error_msg = "Update mode did not find an existing config.yml. Switch to Fresh install and fill/generate the required secrets.".into();
            self.phase = Phase::Error;
            return;
        }
