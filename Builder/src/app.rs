...
use crate::{
    ...
    wizard::{BuildConfig, DbBackend, FieldId, WizardState},
};
...
impl App {
    ...
    pub fn lang(&self) -> Lang {
        *Lang::all().get(self.wizard.lang_idx).unwrap_or(&Lang::En)
    }
    ...
    fn selector_prev(&mut self, fid: FieldId) {
        match fid {
            ...
            FieldId::Language => {
                let l = Lang::all().len();
                self.wizard.lang_idx = (self.wizard.lang_idx + l - 1) % l;
            }
    ...
    fn selector_next(&mut self, fid: FieldId) {
        match fid {
            ...
            FieldId::Language => {
                let l = Lang::all().len();
                self.wizard.lang_idx = (self.wizard.lang_idx + 1) % l;
            }
            fid => self.toggle_or_advance_no_move(fid),
        }
    }
    ...
    fn go_confirm(&mut self) {
        self.config = self.wizard.to_config();
