...
pub fn spawn_uninstall(cfg: BuildConfig) -> (thread::JoinHandle<()>, mpsc::Receiver<InstallMsg>) {
    let (tx, rx) = mpsc::channel::<InstallMsg>();
    let handle = thread::spawn(move || {
        if let Err(e) = run_uninstall(&cfg, &tx) {
...
fn run_uninstall(cfg: &BuildConfig, tx: &mpsc::Sender<InstallMsg>) -> Result<()> {
    let output_dir = cfg.output_path();
    log(tx, format!("Uninstalling KeyBase from: {}", output_dir.display()));

    if !output_dir.exists() {
        bail!("Installation directory does not exist: {}", output_dir.display());
    }
    ...
    let _ = tx.send(InstallMsg::Progress { step: InstallStep::Extracting, done: 0, total: 2 });
    if let Some(existing) = &existing_config {
        let ports = existing_listener_ports(existing, 8080);
        let _ = stop_existing_server(&output_dir, &ports, tx);
    } else {
        terminate_keybase_processes();
    }
    let _ = tx.send(InstallMsg::Progress { step: InstallStep::Extracting, done: 1, total: 2 });
...
fn apply_security_ops(
    cfg: &BuildConfig,
    dir: &Path,
    tx: &mpsc::Sender<InstallMsg>,
) -> Result<()> {
    let any = cfg.sec_reset_pwd
        || cfg.sec_reset_jwt
        || cfg.sec_reset_api
        || cfg.sec_change_username;
    if !any {
        return Ok(());
    }
    ...
    if cfg.sec_change_username && !cfg.sec_new_username.trim().is_empty() {
        ...
        log(tx, format!("  Admin username → '{}'", cfg.sec_new_username.trim()));
    }

    if cfg.sec_reset_pwd && !cfg.sec_new_pwd.is_empty() {
        let hash = bcrypt::hash(&cfg.sec_new_pwd, 12)
