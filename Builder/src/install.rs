...
impl std::fmt::Display for Cancelled {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("installation cancelled by user")
    }
}
...
pub fn cancel_requested(flag: &CancelFlag) -> bool {
    flag.load(Ordering::Relaxed)
}

fn ensure_not_cancelled(flag: &CancelFlag) -> Result<()> {
...
/// Spawn the uninstall pipeline in a background thread.
pub fn spawn_uninstall(
    cfg: BuildConfig,
    cancel: CancelFlag,
) -> (thread::JoinHandle<()>, mpsc::Receiver<InstallMsg>) {
    let (tx, rx) = mpsc::channel::<InstallMsg>();
    let handle = thread::spawn(move || match run_uninstall(&cfg, &tx, &cancel) {
        Ok(()) => {
            let _ = tx.send(InstallMsg::Done);
        }
        Err(e) if is_cancelled(&e) => {
            let _ = tx.send(InstallMsg::Cancelled);
        }
        Err(e) => {
...
fn run_uninstall(
    ...
    log(
        tx,
        format!("Uninstalling KeyBase from: {}", output_dir.display()),
    );
    ...
    let existing_config = if output_dir.join("config.yml").exists() {
        load_existing_config(&output_dir)
    } else {
        None
    };

    log(tx, "Stopping any running KeyBase server…");
    ...
    if let Some(existing) = &existing_config {
        let ports = existing_listener_ports(existing, 8080);
        let _ = stop_existing_server(&output_dir, &ports, tx, cancel);
    } else {
...
pub fn spawn_install(
    ...
    let (tx, rx) = mpsc::channel::<InstallMsg>();
    let handle = thread::spawn(move || match run_install(&cfg, &python, &tx, &cancel) {
        Ok(()) => {
            let _ = tx.send(InstallMsg::Done);
        }
        Err(e) if is_cancelled(&e) => {
...
fn run_install(
    ...
    let downloaded = download_with_progress(&release_urls, tx, cancel)
    ...
    if upgrade_mode {
        if let Some(existing) = &existing_config {
            ...
            stop_existing_server(&output_dir, &ports, tx, cancel)
        ...
        cleanup_directory(&output_dir, &[], cancel)
    ...
    extract_zip(&downloaded.bytes, &output_dir, tx, cancel)
    ...
    create_python_venv(&output_dir, &venv_dir, python, tx, cancel)
    ...
    install_python_packages(&output_dir, &venv_dir, python, tx, cancel)
    ...
    config_gen::write_env(&output_dir, cfg).context("write .env files")?;
    emit_file(tx, ".env");
    ...
    if let Some(existing) = &existing_config {
        maybe_migrate_database(&output_dir, cfg, existing, &venv_py, tx, cancel)
    ...
    // ── 7. Security operations (Update mode only) ─────────────────────────────
    if upgrade_mode {
        apply_security_ops(cfg, &output_dir, tx)?;
    }

    Ok(())
}

fn apply_security_ops(cfg: &BuildConfig, dir: &Path, tx: &mpsc::Sender<InstallMsg>) -> Result<()> {
    let any =
        cfg.sec_reset_pwd || cfg.sec_reset_jwt || cfg.sec_reset_api || cfg.sec_change_username;
    if !any {
        return Ok(());
    }
    log(tx, "Applying security operations…");

    let env_path = dir.join(".env");
    let mut content = if env_path.exists() {
        fs::read_to_string(&env_path).context("Could not read .env")?
    } else {
        String::new()
    };

    if cfg.sec_change_username && !cfg.sec_new_username.trim().is_empty() {
        content = set_env_key(content, "KEYBASE_ADMIN_USER", cfg.sec_new_username.trim());
        log(
            tx,
            format!("  Admin username → '{}'", cfg.sec_new_username.trim()),
        );
    }

    if cfg.sec_reset_pwd && !cfg.sec_new_pwd.is_empty() {
        let hash = bcrypt::hash(&cfg.sec_new_pwd, 12).context("Failed to hash admin password")?;
        content = set_env_key(content, "KEYBASE_ADMIN_PASSWORD_HASH", &hash);
        log(tx, "  Admin password hash updated.");
    }

    if cfg.sec_reset_jwt && !cfg.sec_new_jwt.is_empty() {
        content = set_env_key(content, "KEYBASE_SESSION_SECRET", &cfg.sec_new_jwt);
        log(tx, "  Session / JWT secret rotated.");
    }

    if cfg.sec_reset_api && !cfg.sec_new_api.is_empty() {
        content = set_env_key(content, "KEYBASE_API_MASTER_KEY", &cfg.sec_new_api);
        log(tx, "  API master key rotated.");
    }

    fs::write(&env_path, content).context("Failed to write updated .env")?;
    log(tx, "Security operations applied successfully.");
    Ok(())
}

fn set_env_key(content: String, key: &str, value: &str) -> String {
    let search_uncommented = format!("{key}=");
    let search_commented = format!("# {key}=");
    let replacement = format!("{key}={value}");

    for line in content.lines() {
        let stripped = line.trim_start_matches('#').trim_start();
        if stripped.starts_with(&search_uncommented) {
            // Replace (uncommented or commented)
            return content
                .lines()
                .map(|l| {
                    let s = l.trim_start_matches('#').trim_start();
                    if s.starts_with(&search_uncommented) {
                        replacement.as_str()
                    } else {
                        l
                    }
        ...
        let _ = search_commented; // suppress unused
    }
...
fn install_python_packages(
    ...
    if run_venv_pip_workflow(&venv_py, output_dir, tx, cancel).is_ok() {
        return Ok(());
    }
    ...
    if try_install_linux_pip_package(output_dir, tx, cancel) {
        log(
            tx,
...
fn install_requirements_with_external_pip(
    ...
    for (exe, use_module) in pip_fallback_candidates(bootstrap_python) {
        ...
        match run_cmd(&exe, &arg_refs, output_dir, cancel) {
            Ok(()) => {
            ...
            Err(err) => {
                if is_cancelled(&err) {
                    return Err(vec![format!("{label}: cancelled")]);
                }
...
fn create_python_venv(
    ...
    match run_cmd(python, &primary_refs, output_dir, cancel) {
        Ok(()) => return Ok(()),
        Err(primary_err) => {
            if is_cancelled(&primary_err) {
            ...
            if linux_missing_venv_support(&primary_text) {
                if try_create_venv_without_pip(output_dir, venv_dir, python, tx, cancel) {
                    return Ok(());
                }
                ensure_not_cancelled(cancel)?;

                if try_install_user_virtualenv(python, output_dir, tx, cancel) {
                    log(
                        tx,
                ...
                if try_install_linux_venv_package(python, output_dir, tx, cancel) {
                    log(
                        tx,
...
fn try_create_venv_without_pip(
    ...
    let _ = remove_path(venv_dir, cancel);
    let venv_target = venv_dir.to_string_lossy().to_string();
    ...
    match run_cmd(python, &args, output_dir, cancel) {
        Ok(()) => {
            log(
                tx,
...
fn try_install_user_virtualenv(
    python: &str,
    cwd: &Path,
    tx: &mpsc::Sender<InstallMsg>,
    cancel: &CancelFlag,
    ...
    for (exe, use_module) in pip_fallback_candidates(python) {
        ...
        log(tx, format!("Trying {label}…"));
        match run_cmd(&exe, &args, cwd, cancel) {
            Ok(()) => {
...
fn try_install_linux_venv_package(
    python: &str,
    cwd: &Path,
    tx: &mpsc::Sender<InstallMsg>,
    cancel: &CancelFlag,
    ...
    for command in commands {
        ...
        match run_linux_admin_command(&command.exe, &command.args, cwd, cancel) {
            Ok(()) => {
...
fn try_install_linux_pip_package(
    cwd: &Path,
    tx: &mpsc::Sender<InstallMsg>,
    cancel: &CancelFlag,
) -> bool {
...
fn run_linux_admin_command(
    ...
    if running_as_root() {
        return run_cmd(exe, &refs, cwd, cancel);
    }
    ...
    run_cmd("sudo", &sudo_refs, cwd, cancel).context("sudo non-interactive command failed")
}
...
fn run_cmd_logged(
    ...
    run_cmd(exe, args, cwd, cancel).with_context(|| format!("{label} failed"))?;
    Ok(())
}
...
fn cleanup_directory(output_dir: &Path, keep: &[&str], cancel: &CancelFlag) -> Result<()> {
    if !output_dir.exists() {
        return Ok(());
    }
    for entry in fs::read_dir(output_dir)? {
        ...
        remove_path(&entry.path(), cancel)?;
    }
    Ok(())
}

fn remove_path(path: &Path, cancel: &CancelFlag) -> Result<()> {
    if !path.exists() {
        return Ok(());
    }
    ensure_not_cancelled(cancel)?;
...
fn stop_existing_server(
    ...
    wait_for_ports_free(&ports, Duration::from_secs(15), cancel)?;
    Ok(())
}
...
fn wait_for_ports_free(ports: &[u16], timeout: Duration, cancel: &CancelFlag) -> Result<()> {
    if ports.is_empty() {
        return Ok(());
    }
...
fn maybe_migrate_database(
    ...
    run_cmd(python, &arg_refs, output_dir, cancel).context("database migration command failed")?;
...
fn download_with_progress(
    ...
    for (idx, url) in urls.iter().enumerate() {
        ...
        match download_single(&client, url, tx, cancel) {
...
fn run_cmd(exe: &str, args: &[&str], cwd: &Path, cancel: &CancelFlag) -> Result<()> {
    ensure_not_cancelled(cancel)?;
