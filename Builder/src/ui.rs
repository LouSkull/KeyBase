...
fn render_footer(f: &mut Frame, app: &App, s: &Strings, p: &Palette, area: Rect) {
    ...
    let active_secret = if app.phase == Phase::Wizard {
        app.wizard
            .effective_fields()
...
fn render_confirm(f: &mut Frame, app: &App, s: &Strings, p: &Palette, area: Rect) {
    ...
    if cfg.install_mode == InstallMode::Fresh {
        lines.push(kv("JWT Secret", &secret_status(&cfg.jwt_secret, 32)));
        lines.push(kv("Admin Password", &secret_status(&cfg.admin_password, 8)));
        lines.push(kv(
            "API Master Key",
            &secret_status(&cfg.api_master_key, 16),
        ));
    } else {
        lines.push(kv("Secrets", "existing config preserved"));
    }
    lines.push(kv(
