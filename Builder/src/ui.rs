...
use crate::{
    ...
    selftest::TestResult,
    wizard::{DbBackend, FieldId, LogLevel, TargetOs},
};
...
fn render_wizard_field(
    ...
    let value: String = match fid {
        ...
        FieldId::Database     => DbBackend::all().get(ws.db_sel).map(|d| d.label()).unwrap_or("?").into(),
        ...
        FieldId::TargetOs     => TargetOs::all().get(ws.target_os_sel).map(|t| t.label()).unwrap_or("?").into(),
        FieldId::Workers      => ws.f_workers.buf.clone(),
        FieldId::LogLevel     => LogLevel::all().get(ws.log_level_sel).map(|l| l.label()).unwrap_or("info").into(),
        ...
        FieldId::Theme        => if ws.dark_theme { "Dark".into() } else { "Light".into() },
    };
...
fn render_done(f: &mut Frame, app: &App, s: &Strings, p: &Palette, area: Rect) {
    ...
    let always = ["config.yml", ".env.example", "README.md", "install.log"];
