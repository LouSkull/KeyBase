...
function updateReleaseBannerRoot(root, data) {
  ...
  var pill = root.querySelector('[data-release-pill]');
  var statusNode = root.querySelector('[data-release-status-text]');
  var current = root.querySelector('[data-release-current-version]');
  var latest = root.querySelector('[data-release-latest-version]');
  var published = root.querySelector('[data-release-published-at]');
  var checked = root.querySelector('[data-release-checked-at]');
  var statusText = state === 'available' ? (pill && pill.getAttribute('data-label-available')) : (state === 'up_to_date' ? (pill && pill.getAttribute('data-label-up-to-date')) : (pill && pill.getAttribute('data-label-unavailable')));
  root.dataset.releaseState = state;
  root.classList.toggle('is-update-available', !!data.available);
  root.classList.toggle('is-update-unavailable', state === 'unavailable');
  if (pill) {
    pill.classList.add('status-warn');
    pill.classList.remove('status-ok', 'status-bad', 'status-muted');
    pill.textContent = statusText || (data.available ? 'Update available' : (state === 'up_to_date' ? 'Up to date' : 'Update check unavailable'));
  }
  if (statusNode) statusNode.textContent = pill ? pill.textContent : (data.available ? 'Update available' : (state === 'up_to_date' ? 'Up to date' : 'Update check unavailable'));
  if (current && typeof data.current_version !== 'undefined') current.textContent = data.current_version ? ('v' + String(data.current_version)) : '—';
  if (latest && typeof data.latest_version !== 'undefined') {
    var latestValue = String(data.latest_version || '').trim();
    if (latestValue) latest.textContent = 'v' + latestValue;
    else if (String(data.release_name || '').trim()) latest.textContent = String(data.release_name).trim();
    else latest.textContent = '—';
  }
  if (published && typeof data.published_at_text !== 'undefined') published.textContent = data.published_at_text || '—';
  if (checked && typeof data.checked_at_text !== 'undefined') checked.textContent = data.checked_at_text || '—';
}
...
function setupReleaseBanner(root) {
  ...
  if (isNaN(intervalMs) || intervalMs < 30000) intervalMs = 60000;
